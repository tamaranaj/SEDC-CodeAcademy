...
let IncomesService = class IncomesService {
    ...
    async getIncomes(id) {
        ...
        if (incomes.length === 0) {
            throw new common_1.NotFoundException('Incomes not created.');
        }
        return incomes;
    }
    async findIncome(userId, incomeId) {
        ...
        if (!userIncome) {
            throw new common_1.HttpException(`Income with id: ${incomeId} not found`, 404);
        }
        return income;
    }
    async createIncome(userId, income) {
    ...
    async updateIncome(incomeId, userId, update) {
